...
def gp():
    ...
    print(Fore.GREEN)
    nameSort = input("Sort by name? y or n: ")

    if nameSort == "y":
        sqlTail = 'lname, fname'
    else:
        sqlTail = "SUBSTR(DATE('NOW'), 6)>SUBSTR(YrMoDa,6), SUBSTR(YrMoDa, 6);"

    sql = "SELECT lname, fname, strftime('%Y-%m-%d', YrMoDa), DATE('NOW')\
            -DATE(YrMoDa) FROM prsn ORDER BY " + sqlTail
    ...
    for i in brdays:
        if TnF:
            print(Fore.RED + i[0],i[1],i[2],i[3])
            TnF = not TnF
        else:
            print(Fore.WHITE + i[0],i[1],i[2],i[3])
            TnF = not TnF
...
def mp():
    """make person puts last, first, birthday in via sql"""

    conn = sqlite3.connect('mynotes.sqlite')
    conn.text_factory = str
    db = conn.cursor()

    print(Fore.GREEN + "Last, First, Birthday")

    nameL = input("Last name: ")
    nameF = input("First name: ")
    brday = input("Birth YYYY-mm-dd: ")

    sql = 'INSERT INTO prsn (lname, fname, yrmoda) VALUES (?, ?, ?);' 
    
    tpl = (nameL,nameF,brday)
